    ...
    >
      <div className={`relative overflow-hidden ring-1 ring-inset ring-black/[0.04] ${compact ? 'aspect-[3/4]' : 'aspect-[4/5]'}`}>
        ...
        {showCarousel ? (
          ...
          <div className="grid h-full w-full grid-cols-2 grid-rows-2 gap-px bg-stone-300/80 transition-transform duration-700 ease-out group-hover:scale-[1.02]">
        ...
        {showCarousel && (
          <>
            <button
              ...
              className="absolute left-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-white/90 p-2 shadow-md ring-1 ring-stone-200/80 backdrop-blur-sm transition hover:bg-white"
            ...
            <button
              ...
              className="absolute right-2 top-1/2 z-20 -translate-y-1/2 rounded-full bg-white/90 p-2 shadow-md ring-1 ring-stone-200/80 backdrop-blur-sm transition hover:bg-white"
                ...
                images.map((_, dotIdx) => (
                  <button
                    ...
                    className={`h-2 w-2 shrink-0 rounded-full transition-colors ${
                      currentImage === dotIdx ? 'bg-stone-900' : 'bg-white/50'
                    }`}
      ...
      <div className={`border-t border-stone-100 bg-white/60 text-center backdrop-blur-[2px] ${compact ? 'px-4 pb-5 pt-5' : 'px-6 pb-7 pt-8'}`}>
        <h3 className={`font-serif leading-snug tracking-tight text-stone-900 ${compact ? 'text-base sm:text-[1.05rem]' : 'text-[1.15rem] sm:text-xl'}`}>{title}</h3>
        <p className="mx-auto mt-2 max-w-[28ch] font-sans text-xs uppercase tracking-[0.25em] text-stone-500">{description}</p>
      </div>
    </motion.div>
  );
};
